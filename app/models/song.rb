class Song < ApplicationRecord
  ...
  scope :search_by_all, -> (artist, album, song) { where('songs.name  LIKE ?','%' + song.strip + '%').joins(:artist).where('artists.name  LIKE ?','%' + artist.strip + '%').joins(:album).where('albums.name LIKE ?','%' + album.strip + '%').includes(:album, :artist)}
end
